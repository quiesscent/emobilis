...
from acc.models import InstitutionDoctorProfile, DoctorProfile
...
def increment_appointment_count(sender, instance, created, **kwargs):
    if created:  # Only increment if a new appointment is created
        doctor  = DoctorProfile.objects.get(doctor=instance.doctor)
...
def update_total_appointments(sender, instance, **kwargs):
    ...
    doctor_profile.total_patients_treated += 1
    doctor_profile.save()
